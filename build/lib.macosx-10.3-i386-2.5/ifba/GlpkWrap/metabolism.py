...
class Metabolism(glpk.glpk):
    ...
    def getReactions(self):
        ...
        num = self.getNumCols()
        return [glpki.glp_get_col_name(self.lp, i) for i in range(1, num+1)]
    
    def getMetabolites(self):
        """Returns a list of all metabolite ids in the model."""
        num = self.getNumRows()
        return [glpki.glp_get_row_name(self.lp, i) for i in range(1, num+1)]
    ...
    def getTransporters(self, postfix='_Transp'):
        ...
        def revMatch(str):
            ...
            if patt.search(str):
                ...
                return True
        return [elem for elem in self.reactions if not revMatch(elem)]
