...
def main(argv):
    dict_links = parser.parseKEGGBipartite2Dict(argv[0])
...
if __name__ == '__main__':
    if not len(sys.argv) == 5:
        usage = """Usage:
python static_degree.py [bipartite network file : string] [metabolite mapping file : string] [source directory for essentiality files : string] [target directory for output : string]"""
        print usage
    else:
